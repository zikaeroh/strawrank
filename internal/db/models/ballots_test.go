...
var (
	ballotDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `PollID`: `integer`, `UserXID`: `text`, `UserIP`: `text`, `Votes`: `ARRAYinteger`}
	_             = bytes.MinRead
)
